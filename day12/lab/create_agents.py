...
def wait_for_agent(client, agent_id, desired, timeout=60):
...
def get_or_create_supervisor(bedrock, sub_agent_data, dispatcher_arn, guardrail_id, account_id, role_arn):
    ...
    # Associate sub-agents as collaborators FIRST
    # Get fresh alias ARNs for all sub-agents
    log("  Refreshing sub-agent alias ARNs...")
    for name, info in sub_agent_data.items():
        agent_id = info["id"]
        alias_id = find_alias(bedrock, agent_id, "v1")
        if not alias_id:
            log(f"    WARNING: No v1 alias found for {name}")
            continue
        fresh_arn = f"arn:aws:bedrock:{REGION}:{account_id}:agent-alias/{agent_id}/{alias_id}"
        sub_agent_data[name]["alias_arn"] = fresh_arn
        log(f"    {name}: {fresh_arn}")

    log("  Associating sub-agents as collaborators...")
    for name, info in sub_agent_data.items():
        try:
            log(f"    Associating {name} with ARN: {info['alias_arn']}")
            ...
            log(f"    ERROR with {name}: {str(e)[:200]}")
            raise

    # Now prepare supervisor with collaborators added
    log("  Preparing supervisor with collaborators...")
